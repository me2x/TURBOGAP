...
__global__ void add_init_energy(double* __restrict__ arr, const double value, const int size)
{
  auto tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < size)
      arr[tid] += value;
    ...
    )
{
  ...
  {
    auto local_r =   pow(q[0] -qs[offset],2)            / sigma2_0;
    ...
    if constexpr(type == pol_k)
    {
      ...
      const auto my_energy = pref[offset] * covpp ;
      ...
      if constexpr (do_forces)
      {
        ...
        auto kernel_der = pref[offset] * cov_pp_der ;
...
void cleanup_3bresult_arrays(void** energy_d, void** forces_d, void** virials_d, void** energy_h, void** forces_h, void** virials_h,const hipStream_t s, const bool do_forces, const int n_sites, const int n_sites0)
{
  ...
  hip_check_error( hipMemcpyAsync(*energy_h, *energy_d,n_sites*sizeof(double),hipMemcpyDeviceToHost ,s));
...
//end dbg

 {
    ...
    if (e0 != 0)
    {
      add_init_energy<<<grids,threads,0,s>>>(energies, e0, n_sites);
    }
 }
  hip_check_error( hipDeviceSynchronize());
